...
func printEndpoints(cfg *config.Config) {
	...
	logrus.Infof("  • Get Student Data:  GET  %s/api/v1/student/{id}", baseURL)
	logrus.Infof("  • Generate PDF:      GET  %s/api/v1/student/{id}/pdf-report", baseURL)
	logrus.Infof("  • Save PDF Report:   POST %s/api/v1/student/{id}/save-report", baseURL)
	logrus.Info("")
	logrus.Info("Example usage:")
	logrus.Infof("  curl %s/api/v1/health", baseURL)
	logrus.Infof("  curl %s/api/v1/student/1", baseURL)
	logrus.Infof("  curl -o student_report.pdf %s/api/v1/student/1/pdf-report", baseURL)
} 